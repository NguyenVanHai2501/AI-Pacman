...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # nhận tất cả hành động của trạng thái
        actions = self.getLegalActions(state)
        # giá trị cho trạng thái cuối là 0
        if len(actions) == 0:
          return 0
        else:
          # giá trị của các trạng thái khác là gtri tối đa của các gtri trạng thái
    ...
    def getAction(self, state):
        ...
        action = None
        # đối với essilon tham lam chọn thời gian epsilon hành động ngẫu nhiên
        # và thời gian 1 epsilon tối ưu
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        # đối với mỗi extracting fuatures (trích xuất tính năng)cho trạng thái và trọng số tương ứng của action
        # nên được trích xuất trả về giá trị của 1 trạng thái đó là tổng (f_i (s, a) * w_i)
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        # cần cập nhật tất cả các trọng số (weights) cho hành động (action)
